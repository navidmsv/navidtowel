...
function setupDesktopCategoryMenu() {

const categoryMenus =
    document.querySelectorAll(".nav-category");
if (!categoryMenus.length) return;
categoryMenus.forEach(function (category) {
    const button =
        category.querySelector(".nav-category-btn");
    const dropdown =
        category.querySelector(".category-dropdown");
    ...
    button.addEventListener(
        ...
        function (event) {
            ...
            event.stopPropagation();
            /* بستن منوهای دیگر */
            categoryMenus.forEach(function (otherCategory) {
